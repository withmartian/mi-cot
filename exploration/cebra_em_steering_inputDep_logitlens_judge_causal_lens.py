"""
CEBRA-EM steering with judge-defined state annotations — **causal-style logit lens**.

Same steering + OpenAI judge protocol as ``cebra_em_steering_inputDep_logitlens_judge_only.py``,
but logit-lens geometry matches ``cebra_causal.py``:

- **Unembedding:** ``lm_head.weight`` (explicit causal-style readout).
- **Activations:** **StandardScaler**-normalized hidden states for centroids and pre/post lens.

Latent dynamics use **CEBRA + SLDS EM** (``cebra_EM``), not MoE.
"""
...
try:
    ...
    raise ImportError("Failed to import `cebra_EM.py`.") from e
...
class SteerConfig:
    ...
    save_dir: str = "cebra_em_steering_logitlens_judge_causal_lens"
    ...
    hf_fallback_cache_name: str = "sds_hf500_em_causal_lens_qwen14b_l47.pkl"
    # Large on-disk pickle of EM weights + steering cache; optional for batch sweeps.
    save_steer_artifacts_pkl: bool = True


def build_cli_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            "CEBRA-EM steering with causal-style logit lens; OpenAI annotates regimes as reasoning stages "
            "(cebra_EM.STAGES), then judges post-steering readouts."
        )
    )
    ...
    p.add_argument(
        ...
        default=None,
        metavar="N",
        help="SLDS EM iterations (default: SteerConfig.em_iters).",
    )
    ...
    p.add_argument(
        ...
        action="store_true",
        help="Do not write steer_cebra_em_logitlens_judge_causal_lens_artifacts.pkl (saves disk).",
    )
...
def config_from_cli_args(cfg: SteerConfig, args: argparse.Namespace) -> SteerConfig:
    ...
    if getattr(args, "em_iters", None) is not None:
        c = replace(c, em_iters=int(args.em_iters))
    ...
    if getattr(args, "no_save_steer_artifacts_pkl", False):
        c = replace(c, save_steer_artifacts_pkl=False)
    return c
...
def resolve_logit_lens_device(explicit: Optional[str]) -> str:
    """Return ``cuda`` or ``cpu`` for logit-lens tensors (falls back to CPU if CUDA is unavailable)."""
    import torch
    ...
    if v == "cuda":
        if not torch.cuda.is_available():
            print(
                "[logit_lens] WARNING: logit_lens_device='cuda' requested but torch.cuda.is_available() is False. "
                "Falling back to CPU.",
                flush=True,
            )
            return "cpu"
...
def _reasoning_stage_annotation_system_prompt() -> str:
    """Paper-aligned guidance (cf. ``4_expms`` functional specialization + ``cebra_EM.STAGES``)."""
    ...
    return (
        ...
        "- If the listed tokens contain **non-English characters** (e.g., Chinese / Japanese / Korean / other scripts), "
        "translate them into their **English meaning** before inferring the reasoning step role "
        "(do not treat raw glyphs as meaningless; use best-effort translation implied by context).\n"
    ...
    *,
    ...
    with open(tex_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            ...
            "\\textbf{CEBRA-EM} latent regimes on "
...
def write_state_annotations_tex(save_dir: str, state_annotations: Dict[Any, dict]) -> str:
    ...
    with open(tex_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            ...
            "\\texttt{aligned\\_stage} matches \\texttt{cebra\\_EM.STAGES} when set).}\n"
        )
...
def run_pipeline(cfg: SteerConfig) -> Tuple[str, dict]:
    ...
    appendix_raw_lens_tex = write_regime_lens_raw_centroids_appendix_tex(
        ...
        cfg.k_regimes,
        dataset_name="GSM8K",
        model_col="Reasoning (EM)",
        n_rows=len(all_features),
        label="tab:regime-lens-em-qwen14b-gsm8k-causal-lens",
    )
        ...
        json.dump(
            {
                ...
                "latent_discovery": "CEBRA_plus_EM",
    ...
    if getattr(cfg, "save_steer_artifacts_pkl", True):
        out_path = os.path.join(cfg.save_dir, "steer_cebra_em_logitlens_judge_causal_lens_artifacts.pkl")
...
if __name__ == "__main__":  # pragma: no cover
    ...
    if out_file:
        ...
        print("Steer artifacts pickle skipped (--no-save-steer-artifacts-pkl).")
