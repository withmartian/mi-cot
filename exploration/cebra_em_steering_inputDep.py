...
def diffusion_postprocess_placeholder(acts_edit: np.ndarray) -> np.ndarray:
    ...
    (see generative latent prior papers) can snap the vector back to plausible states before use.
...
Qualitative regime–stage specialization (SDS early-results PDF; for reference only, indices s_t/k* are EM ids):
{_SDS_EARLY_RESULTS_REGIME_STAGE_SPECIALIZATION}
