...
# checkpoint_dir = "rpc_dataset_gsm8k_llama_layer_final_500"
# os.makedirs(checkpoint_dir, exist_ok=True)
# dtype = torch.bfloat16

# model_ft_name = "deepseek-ai/DeepSeek-R1-Distill-Llama-8B"
# model_base_name = "meta-llama/Llama-3.1-8B"
# dataset_path = "openai/gsm8k" #"HuggingFaceH4/MATH-500" #
# number_cots = 500
# EXTRACT_LAYER = 31
...
device = "cuda" if torch.cuda.is_available() else "cpu"
checkpoint_dir = "rpc_dataset_math500_layer_20_qwen1_5"
os.makedirs(checkpoint_dir, exist_ok=True)
dtype = torch.bfloat16

model_ft_name = "deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B"
...
if not hasattr(hf_model.config, "rope_theta"):
    ...
    hf_model.config.rope_theta = getattr(hf_model.config, "rope_base", 1000000.0)
    print(f"DEBUG: Manually set rope_theta to {hf_model.config.rope_theta}")

# 2. Pass the modified config EXPLICITLY into the loader
# This prevents TransformerLens from re-loading the config from the model name
model = HookedTransformer.from_pretrained_no_processing(
    model_base_name, 
    hf_model=hf_model, 
    device=device, 
    dtype=dtype,
    hf_config=hf_model.config  # <--- Add this line
)
...
# --- MAIN EXTRACTION ---
print("Loading dataset...")
dataset = load_dataset(dataset_path, 'default', split=f"test[:{number_cots}]")
